...
namespace DimensionCollapse
{
    public class Bullet : MonoBehaviour
    {
        ...
        public Vector3 lastPosition = new Vector3(float.MaxValue, float.MaxValue, float.MaxValue); //此条记录上一次的位置点，辅助实现防止子弹越过物体
        private float length;    //test
        private Ray ray; //test
            ...
            if (!isDead) //活着的子弹才进行是否超过生命周期判定
            {
                ...
                if (this.GetComponent<Rigidbody>().velocity.magnitude > 50f)
                {
                    ...
                    if (lastPosition.x < float.MaxValue && bulletRigidbody.velocity.magnitude > HIGH_SPEED)
                    {
                        ...
                        length = (transform.position - lastPosition).magnitude;
                        RaycastHit hitInfo;
